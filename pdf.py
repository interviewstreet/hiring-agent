...
class PDFHandler:
    """Handles PDF parsing, text extraction, and section extraction."""
    ...
    def extract_text_from_pdf(self, pdf_path: str) -> Optional[str]:
        """Extract text from a PDF file with validation and logging."""

        # ✅ Validation at the very start
        if not os.path.exists(pdf_path):
            raise FileNotFoundError(f"⚠️ PDF file not found: {pdf_path}")

        if not pdf_path.lower().endswith(".pdf"):
            raise ValueError(f"⚠️ Invalid file type: {pdf_path}. Please provide a PDF file.")
        ...
        try:
            ...
            for i, page in enumerate(doc):
                ...
                if page_text.strip():
                    ...
                    logger.warning(f"⚠️ Page {i+1} has no readable text (maybe scanned image).")

            resume_text = "\n".join(text_parts).strip()

            if not resume_text:
                logger.error(f"❌ No extractable text found in '{pdf_path}'. File may be scanned.")
                return None

            logger.debug(f"✅ Extracted {len(resume_text)} characters from '{pdf_path}'.")
            return resume_text
    ...
    # ----------------------
    # Existing LLM section extraction functions
    # ----------------------
    ...
    def _call_llm_for_section(
        ...
        try:
            ...
            try:
                ...
                end_time = time.time()
                logger.debug(f"⏱️ Total time for section extraction: {end_time - start_time:.2f} sec")
    ...
    def _extract_all_sections_separately(self, text_content: str) -> Optional[JSONResume]:
        ...
        complete_resume = {section: None for section in [
        ...
        try:
            ...
            logger.info(f"⏱️ Total time for separate section extraction: {time.time() - start_time:.2f} seconds")
            return json_resume
