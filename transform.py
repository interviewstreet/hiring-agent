...
def fetch_profile(profiles, network_names, prefix):
    ...
    for network in network_names:
        profile = next((p for p in profiles if p.network and p.network.lower() == network.lower()), None)
        if profile:
            return profile    
...
def transform_evaluation_response(file_name=None, resume_data=None, github_data=None, evaluation=None):
    ...
    if resume_data and hasattr(resume_data, 'basics') and resume_data.basics:
        ...
        if basics.profiles:
            ...
            twitter_profile = fetch_profile(basics.profiles, ['twitter', 'x'], 'twitter')
            dev_profile = fetch_profile(basics.profiles, ['dev community', 'dev'], 'dev')
            ...
            if github_profile:
                ...
                csv_row['github_username'] = github_profile.username if github_profile.username else ''
            else:
                csv_row['github_url'] = ''
            ...
            if twitter_profile:
                ...
                csv_row['twitter_username'] = twitter_profile.username if twitter_profile.username else ''
            else:
                csv_row['twitter_url'] = ''
            ...
            if dev_profile:
                ...
                csv_row['dev_username'] = dev_profile.username if dev_profile.username else ''
            else:
                csv_row['dev_url'] = ''
            ...
            if behance_profile:
                ...
                csv_row['behance_username'] = behance_profile.username if behance_profile.username else ''
            else:
                csv_row['behance_url'] = ''
    ...
    if evaluation and hasattr(evaluation, 'scores'):
        ...
        total_score = (scores.open_source.score + scores.self_projects.score + 
                      scores.production.score + scores.technical_skills.score)
        total_max = (scores.open_source.max + scores.self_projects.max + 
                    scores.production.max + scores.technical_skills.max)
